...
export class ClientRequestsComponent implements OnInit {
  ...
  sortedCustData:any
  sortCustData() {
    this.sortedCustData = this.clientData.sort((a: any, b: any) => {
      if (a.assignedQuantity === a.quantity && b.assignedQuantity !== b.quantity) return 1; // 'Assigned' to bottom
      if (a.assignedQuantity !== a.quantity && b.assignedQuantity === b.quantity) return -1;
      if (a.assignedQuantity > 0 && b.assignedQuantity === 0) return 1; // 'Partially Assigned' to middle
      if (a.assignedQuantity === 0 && b.assignedQuantity > 0) return -1;
      return 0; // Same state, no change
    });
  }
  ...
  console.log(item)
    this.inventorySer.listInventoryForSending(item).subscribe((res: any) => {
      // console.log(res);
  ...
  updateInventoryStatusForFGSending() {
    this.assignedObj.modifiedBy = this.user?.UserId
    this.inventorySer.updateInventoryStatusForFGSending(this.assignedObj).subscribe((res:any)=> {
      // console.log();
  ...
  getMetadata() {
    ...
    data?.forEach((item: any) => {
      if(item.type === 48) {
        this.Weavers_Inventory_Status= item.metadata;
        // console.log(this.Weavers_Inventory_Status)
      }
    })
  }
  siteNg: any;
